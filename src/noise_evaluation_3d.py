...
def get_input_channels_3d(X):
    """
    Infer channel count for 3D data.

    Possible shapes:
    (N, D, H, W) -> 1 channel
    (N, C, D, H, W) -> C channels
    (N, D, H, W, C) -> C channels
    """
    X = np.asarray(X)
    ...
    raise ValueError(f"Cannot infer 3D channels from shape {X.shape}")
...
def train_clean_3d_model(X_train, y_train, multi_label, epochs=5, batch_size=16, lr=3e-4):
    ...
    print(f"Training clean 3D CNN model on: {DEVICE}")
...
def evaluate_3d_model(model, X_eval, y_eval, multi_label, batch_size=16):
    model.eval()

    eval_ds = prepare_tensors_3d(
        X_eval,
        y_eval,
        multi_label=multi_label,
    )
...
def save_noise_results(data_flag, results):
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"{data_flag}_noise_evaluation_3d.csv")
    ...
    print(f"\nSaved 3D noise evaluation CSV to: {csv_path}")


def run_noise_evaluation_3d(data_flag="adrenalmnist3d", epochs=5):
    print("=" * 64)
    print(f"3D Noise Robustness Evaluation: {data_flag}")
    print("=" * 64)
    ...
    print("\nEvaluating clean validation volumes...")
    clean_auc, clean_acc = evaluate_3d_model(model, X_val, y_val, multi_label)
    results.append(("Clean", clean_auc, clean_acc))

    print("Evaluating Gaussian noise validation volumes...")
    X_gaussian = add_gaussian_noise(X_val, std=0.10)
    gaussian_auc, gaussian_acc = evaluate_3d_model(model, X_gaussian, y_val, multi_label)
    results.append(("Gaussian Noise std=0.10", gaussian_auc, gaussian_acc))

    print("Evaluating salt-and-pepper noise validation volumes...")
    X_sp = add_salt_pepper_noise(X_val, amount=0.05)
    sp_auc, sp_acc = evaluate_3d_model(model, X_sp, y_val, multi_label)
    results.append(("Salt-Pepper Noise amount=0.05", sp_auc, sp_acc))

    print("Evaluating speckle noise validation volumes...")
    X_speckle = add_speckle_noise(X_val, std=0.10)
    speckle_auc, speckle_acc = evaluate_3d_model(model, X_speckle, y_val, multi_label)
    results.append(("Speckle Noise std=0.10", speckle_auc, speckle_acc))

    elapsed = time.time() - start

    print("\n3D Noise Evaluation Results")
    print("=" * 64)
...
def main():
    run_noise_evaluation_3d(data_flag="adrenalmnist3d", epochs=5)
