# src/analyze_results_2d.py
# Analyze saved 2D CNN result files and generate summary tables + graphs.
...
RESULTS_DIR = "models_2d"
OUTPUT_CSV = os.path.join(RESULTS_DIR, "summary_results_2d.csv")
    ...
    for filename in os.listdir(RESULTS_DIR):
        if filename.endswith(".joblib"):
            ...
            try:
                ...
                dataset = result.get("dataset", filename.replace("_cnn2d_results.joblib", ""))
                auc = result.get("auc", None)
                accuracy = result.get("accuracy", result.get("acc", None))
                method = result.get("method", "cnn2d")
...
def plot_metric(results, metric, ylabel, filename):
    ...
    plt.xlabel("Dataset")
    plt.ylabel(ylabel)
    plt.title(f"2D CNN {ylabel} by Dataset")
...
def plot_training_time(results):
    ...
    plt.title("2D CNN Training Time by Dataset")
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
...
def main():
    ...
    if not results:
        print("No valid 2D result files found.")
        return
